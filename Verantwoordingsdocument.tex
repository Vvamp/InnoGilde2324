...
\begin{enumerate}
  \item \href{https://archlinux.org/}{Arch Linux}. - Dit besturingssysteem had ik al op mijn laptop staan
  \item \href{https://manjaro.org/}{Manjaro} - Manjaro is een Linux distrobutie gebaseerd op Arch Linux. Het wordt geprezen voor simpliciteit en gebruiksvriendelijkheid. 
  \item \href{https://ubuntu.com/}{Ubuntu} - Ubuntu is gebaseerd op Debian. 
...
Ik heb specifieke eisen aan een besturingssysteem. 
Zo heb ik graag een snel systeem met toegang tot de nieuwste packages, maar is gebruiksvriendelijkheid helemaal geen prioriteit.
Deze heb ik voor het gemak getoond in het volgende taartdiagram. 
\begin{figure}
  \centering
\begin{tikzpicture}
...
In Linux distributies worden programma's vooral gedownload vanaf package managers. Deze worden 

\begin{table}[h]
  \centering
  \begin{tabular}{|l|c|}
    \hline
    \textbf{Eis} & \textbf{Gewicht} \\
    \hline
    Snelheid & 2 \\
    Nieuwe Packages & 3 \\
    ...
    \hline
  \end{tabular}
  \caption{Gewichten per eis}
  \label{tab:spec_weights}
\end{table}

In onderstaande tabel heb ik een lijst gemaakt van een aantal Linux distributies die mij interessant leken.

\begin{table}[h]
  \centering
  \begin{tabular}{|l|c|c|c|c|}
    \hline
    \textbf{Besturingssysteem} & \textbf{Snelheid} & \textbf{Nieuwe Packages} & \textbf{Gebruiksvriendelijkheid} & \textbf{Stabiliteit} \\
    \hline
    Arch Linux & ++ & ++ & - & + \\
    \hline
    Ubuntu & + & - & ++ & ++ \\
    ...
    \hline
  \end{tabular}
  \caption{Ratings for each OS based on specs}
...
\par\smallskip
Native geeft mij dus het meeste opslag, de snelste 
